...
class MultiSignal(gym.Env):
    ...
    gui=False, end_time=3600, step_length=10, yellow_length=4, max_distance=20, lights='', \
    log_dir='/', libsumo=False, warmup=0):
        ...
        for ts in self.all_ts_ids:
            ...
            self.signals[ts].observe(self.step_length)
    ...
    def step(self, signal_act, veh_act):
        ...
        for step in range(self.step_length - self.yellow_length):
            ...
            self.step_sim()
        for signal in self.signal_ids:
            self.signals[signal].observe(self.step_length)
        ...
        for cav in veh_act:
            if cav in self.cav_ids:
                ...
                if edge in self.direction_encoding:
                    ...
                    print("TIME", self.sumo.simulation.getTime(), "CAV:", cav, "CURR:", e, "PREV:", route, "NEW", new_route)
